...
import { Home, Map, Camera, Bot, Settings, Waves, AlertTriangle } from 'lucide-react';
...
export default function BottomNav({ variant }: { variant: Variant }) {
  ...
  // Sidebar (desktop) gets an extra Alerts link — useful for ops/researchers
  // who'll spend time monitoring the feed; mobile nav stays focused on
  // the most-used 5 actions.
    ...
    return (
      <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-64 flex-col p-6 z-20
        ...
        <nav className="flex flex-col gap-0.5">
          ...
          {/* Divider */}
          <div className="my-3 h-px bg-grass/20 dark:bg-[#374151]" />

          {/* Sidebar-only extras (desktop only) */}
          {SIDEBAR_EXTRAS.map(({ href, label, icon: Icon }) => {
            const active = pathname === href;
            return (
              <Link
                key={href}
