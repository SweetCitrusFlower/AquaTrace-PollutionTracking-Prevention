...
export default function CameraPage() {
  ...
  // Tracks whether the latest submission successfully reached the DB
  // (vs. only saved locally because backend was offline).
  const [reportPersisted, setReportPersisted] = useState<boolean | null>(null);
  ...
  const handleSubmit = () => {
    ...
    // Award tokens locally for immediate UX (works offline).
    if (user) {
      updateUser({
    ...
    setReportPersisted(null); // pending
    setStage('success');

    // Fire-and-forget POST to backend. UX has already moved on.
    if (location) {
      submitReport({
        user_id: user?.id,
        lat: location.lat,
        lng: location.lng,
        smell_score: report.odor === 'Rotten Eggs / Sulfur' ? 5
                   : report.odor === 'Sewage'              ? 4
                   : report.odor === 'Chemical'            ? 4
                   : report.odor === 'Fishy'               ? 3
                   : report.odor === 'Odorless'            ? 1 : undefined,
        water_flow: report.flow === 'Low'    ? 'stagnant'
                  : report.flow === 'Normal' ? 'slow'
                  : report.flow === 'High'   ? 'fast'
                  : report.flow === 'Flood'  ? 'fast' : undefined,
        human_activity: report.activity.length > 0 ? report.activity.join(', ') : undefined,
      })
        .then(res => setReportPersisted(res.ok))
        .catch(() => setReportPersisted(false));
    } else {
      setReportPersisted(false); // no location = can't persist
    }
  };
  ...
  return (
    <div className="px-4 md:px-10 py-6 max-w-2xl mx-auto">
      ...
      {stage === 'success' && <SuccessStage onReset={reset} persisted={reportPersisted} />}
    </div>
  );
}
...
/* ========== STAGE 3: SUCCESS ========== */
function SuccessStage({ onReset, persisted }:
  { onReset: () => void; persisted: boolean | null }) {
  return (
    <div className="text-center py-10">
      ...
      <p className="text-dusk/70 mb-4">Thank you for protecting the Danube 💚</p>

      {/* Persistence status pill — gives clear feedback when offline */}
      <div className="mb-6 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mx-auto">
        {persisted === null && (
        ...
        {persisted === true && (
          <span className="inline-flex items-center gap-1.5 bg-green-50 text-green-700 px-3 py-1.5 rounded-full border border-green-200">
            <Check className="w-3 h-3" /> Synced to database
          </span>
        )}
        {persisted === false && (
          <span className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full border border-amber-200">
