...
import { Mail, Lock, User as UserIcon, Loader2, Droplets } from 'lucide-react';
    ...
    <div className="min-h-[calc(100vh-12rem)] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-3xl font-bold text-dusk-dark">Join the guardians</h1>
          <p className="text-dusk/70 text-sm mt-1">Sign up and start earning tokens for every report.</p>
        </div>

        <form onSubmit={submit} className="card-eco space-y-4">
          <Field label="Username" icon={<UserIcon className="w-4 h-4" />}>
            <input type="text" value={username} onChange={e => setUsername(e.target.value)}
              placeholder="riverhero42"
          ...
          <p className="text-[11px] text-dusk/60 text-center">
        ...
        <p className="text-center text-sm text-dusk/70 mt-6">
          ...
          <Link href="/login" className="text-water-dark font-semibold hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
